...
#' finite population sizes and non-random mating over many generations
#'
#' @description \code{finite_non_rand2} simulates finite Mendelian populations with non-random mating over many generations.
...
#' @param outcrossingrate_nr Outcrossing rate
...
#' @details \code{finite_non_rand} returns three plots:
...
finite_non_rand2<-function (outcrossingrate_nr,p1g2_nr,p2g2_nr,alleles,ND_nr,generations_nr,n_rep) {
  ...
  for (jj in 1:n_rep) {
    ...
    for (i in 1:generations_nr) {
      ...
      #first generation
       if (i==1){
      parent_nr1<-cbind(sample(alleles,ND_nr,replace=TRUE,prob=c(p1g2_nr1,p2g2_nr1)),sample(alleles,ND_nr,replace=TRUE,prob=c(p1g2_nr1,p2g2_nr1)))
      for (j in 1:ND_nr) {
      matingprob_nr<-runif(1)
      g2A1_nr[j]<-sample(parent_nr1[j,],1)
      g2A2_nr[j]<-if (matingprob_nr>outcrossingrate_nr) {
        sample(parent_nr1[j,],1)
      } else {
        sample(parent_nr1,1)
      }
      }
      parent_nr<-cbind(g2A1_nr,g2A2_nr)
      } else {

      for (j in 1:ND_nr) {
        matingprob_nr<-runif(1)
        g2A1_nr[j]<-sample(parent_nr[j,],1)
        g2A2_nr[j]<-if (matingprob_nr>outcrossingrate_nr) {
          sample(parent_nr[j,],1)
        } else {
          sample(parent_nr,1)
        }
      }
